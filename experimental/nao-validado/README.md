...
> Eles permanecem em seus diretórios dentro de `experimental/` (continuam funcionando);
> este registro apenas os **identifica**. Cada um tem um banner `[NAO VALIDADO]` no topo
> do arquivo.
...
|---|---|---|
| `Diagnostico-GPO-Client.ps1` | experimental/active-directory/ | exige domínio AD; nunca executado (ver `tests/lab-ad/`) |
| `Testa-Repara-ContaMaquinaAD.ps1` | experimental/active-directory/ | exige domínio AD; nunca executado |
| `Configurar-Idioma-Regional.ps1` | experimental/configuration/ | sem execução real; altera idioma/regional do SO |
| `Diagnostico-Driver-Grafico.ps1` | experimental/diagnostics/ | sem execução real |
| `Verificar-Atualizacoes-Hardware.ps1` | experimental/diagnostics/ | sem execução real (somente leitura) |
| `Testar-Conectividade-Internet.ps1` | experimental/diagnostics/networking/ | sem execução real |
| `Backup-Restaurar-Drivers.ps1` | experimental/maintenance/ | sem execução real; usa DISM/pnputil |
| `Limpeza-WinSxS.ps1` | experimental/maintenance/ | sem execução real; usa DISM no Component Store |
| `Preparar-Imagem-Windows.ps1` | experimental/maintenance/ | sem execução real; sysprep (alto risco) |
| `Remover-Perfis-Inativos.ps1` | experimental/utilities/ | sem execução real; remove perfis de usuário (alto risco) |
